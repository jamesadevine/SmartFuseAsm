...
smplOvrfMx		EQU	0x02	;sample overflow max
;smplOvrfMx		EQU	0x02
smplOvrfCr		EQU	0x31		;sample overflow current
smplOvrfMa		EQU	0x75	;sample overflow match
;smplOvrfMa		EQU	0x01	

;adc result H and L used to display on LEDs
adcResultH		EQU	0x70
adcResultL		EQU	0x71
...
INIT:
	...
	;call	INT_VECTOR
	call	DELAYMS
	;reset porta and clear adcResult
	clrf	PORTA
	clrf	PORTC
	clrf	adcResultH
	clrf	adcResultL
...
;-----------------------LOOP---------------------
;converts adc and displays the result on the LEDS	
INPUTLOOP:
	banksel	PORTA
	movlw	0x01
	movwf	PORTA
...
INT_VECTOR: 
	banksel PORTA	
	movlw 	0x80		;move 0xAA onto PORTA to visualize communication
	movwf	PORTA
	call	SETUPFORCOMMS
